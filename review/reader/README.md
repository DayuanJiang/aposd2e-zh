...
## Chapter Opening Maps and Closing Storylines (2026-09-21)

Every chapter now opens with a map (`chNN-map`) and closes with a storyline
(`chNN-story`), 44 figures generated by `bin/chapter_figures.py` from
`docs/.vuepress/reader/chapters/chNN.json`; traditional variants come from
`tools/localize.py`. The reader labels them 本章概览 and 本章回顾.

Review and revision are recorded in `chapter-figures-review.json` (summary,
decisions, application and verification) and the four batch reports
`chapter-figures-review-01-06.json`, `-07-11.json`, `-12-16.json`, `-17-22.json`.
Four independent reviewer agents judged all 44 figures against the chapter text and
the reader-visualization rules: no P1, 35 P2, 106 P3; 39 figures marked `revise`,
...
Python tests, `browser.mjs --gallery` over 388 assets in both themes with 0 geometry
candidates, and a visual sample of desktop and mobile figures. This is explanatory
design judgment, not a reader experiment.
