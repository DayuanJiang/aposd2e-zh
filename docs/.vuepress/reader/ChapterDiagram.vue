...
import { BookOpen, ChevronDown, Expand } from '@lucide/vue'
...
<template>
  <figure v-if="meta" class="chapter-guide" :data-chapter="chapter" :data-diagram="diagramId" :data-diagram-ready="Boolean(svg)">
    <details open>
      <summary><span class="guide-kind"><BookOpen />{{ traditional ? '譯者圖解' : '译者图解' }}</span><span>{{ title }}</span><ChevronDown class="guide-chevron" /></summary>
      <div class="diagram-toolbar"><button class="reader-icon-button" type="button" :title="traditional ? '放大圖解' : '放大图解'" :aria-label="traditional ? '放大圖解' : '放大图解'" :disabled="!svg" @click="modal?.open($event, raw)"><Expand /></button></div>
